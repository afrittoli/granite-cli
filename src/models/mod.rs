...
impl ModelSource {
    ...
    pub fn get(&self, model_id: &str) -> anyhow::Result<Arc<dyn Model>> {
        ...
        // the id has one instance however the calls interleave, and one
        // proxy route per model.
        Ok(self
            .cache
            .lock()
            .unwrap()
...
mod tests {
    ...
    #[tokio::test]
    async fn get_routes_through_proxy_and_registers_a_route_when_a_handle_is_active() {
        use crate::config::{Config, ModelConfig, ProviderConfig};
